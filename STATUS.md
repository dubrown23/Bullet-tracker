...
**Last updated:** 2026-05-13 (`bt-0003` **in build through Wave 4.5; smoke build clean** — Waves 0 / 1 / 2 / 3 / 3.5 / 4 / 4.5 all shipped to `swiftdata-phase2` branch with commits. Wave 4 device-test (Today / Dashboard / Journal / Widget render / Widget tap) passed functionally — no Wave-3.5-class predicate-shape rejection — but flagged a perf regression: cold-render lag on Today caused by an N+1 read pattern (each card's body issued ~10 single-row predicate fetches: 3× for `completionState` accesses + 7× for the 7-day mini-dots). Wave 4.5 hotfix: added `HabitStore.completionState(for:entry:)` sibling that takes a pre-fetched entry, and an `entriesLast7Days: [Date: HabitEntry]` prefetch in `TodayHabitRowView` — one indexed range fetch returning up to 7 rows replaces the ~10 single-row queries. Net +24 lines across 2 files. Wave 4.5 device-confirmed loadable; lag-resolution verification pending Dustin's read. CloudKit remains disabled (`cloudKitDatabase: .none`); re-enabled at Wave 5 ship. Punted: `Bullet TrackerTests/Bullet_TrackerTests.swift` cleanup before Wave 5.)
...
- `bt-0003` — SwiftData Phase 2. **In build on `swiftdata-phase2` branch through Wave 4.5; smoke build clean, lag-resolution confirmation pending.** Waves 0 (backup-restore `@MainActor`), 1 (drop dormant machinery + Path 3 recovery validated), 2 (combined schema changes), 3 (consumer rewrites — 15 files), 3.5 (`HabitEntry.details` storage hotfix — computed property over stored `String?` after bt-0004-class SwiftData rejection), 4 (hot-path predicate rewrites — 5 reads across `HabitDataRepository` + widget, +66 lines), 4.5 (Today-card N+1 hotfix — prefetched 7-day window per card, +24 lines) all shipped to branch with commits. CloudKit temporarily disabled (`cloudKitDatabase: .none`) for build duration; re-enabled at Wave 5 ship. Wave 4.5 cold-load lag confirmation is the last gate before Wave 5.
...
1. **bt-0003 Wave 5 — ship.** Re-enable CloudKit (`cloudKitDatabase: .automatic` in `DataStore.swift`). Take a fresh backup; wipe iCloud's side via Settings → name → iCloud → Manage Account Storage (if Bullet Tracker appears there); reinstall + restore; cross-device sync verification if available. Merge `swiftdata-phase2` → `main`. **Pre-ship cleanup:** `Bullet TrackerTests/Bullet_TrackerTests.swift` (punted in Wave 3 — references legacy `entry.completionState > 0` patterns that won't compile against the new model).
2. (Independent, anytime) Repo-layout cleanup — finish the folder refactor, delete `WidgetEnums.txt` / committed `.DS_Store`s, archive the old `Documentation/` files.
