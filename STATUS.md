...
- `bt-0002` — **SwiftData migration, Phase 1** (engine swap, model shape unchanged). On the `swiftdata-migration` branch. Scope locked (~22 Core-Data-touching `.swift` + the model XML + the project file). **Strategy "(b)": one focused data-layer push** — no Core-Data-coexistence period (the earlier "turn off codegen, gradual migrate" plan was wrong: `@Model` types aren't `NSManagedObject` drop-ins — see bt-0002 Decision log). App stays red until the last wave; JSON backup is the safety net; merge when it compiles + works.
  ...
  - **Wave 3b done** (`bbe8485`): `Journal/DayJournalView.swift` 843→846 lines. Same VM-stored-`modelContext` pattern as Wave 3a; `DayJournalViewModel` reads via `FetchDescriptor<HabitEntry>()` / `FetchDescriptor<Note>()` + in-Swift date-window filter (no `#Predicate` yet, mirrors Wave 3a — same workaround for watch-item (e)); `buildExportData(for:)` does one fetch up front and filters per-day in Swift. Writes use `Note(date:content:)` + `context.insert` (was `Note(context:)` Core-Data init); `context.delete(...)` API unchanged on `ModelContext`. After this, only `Data/CoreDataManager.swift`, `Data/CoreDataManager+HabitDetails.swift`, and `BulletTrackerWidgets/BulletTrackerWidgetsBundle.swift` still `import CoreData` — all three covered by Wave 4 + post-Wave-4 deletion.
  - **Next session = Wave 4 (widget):** rewrite `CompleteHabitIntent` + the timeline provider against a `ModelContainer` built in the extension (same App-Group store URL as `DataStore.shared` but a fresh container, since `ModelContainer` is per-process); revisit `WidgetEnums.swift` stubs. Then delete `Data/CoreDataManager.swift` + `Data/CoreDataManager+HabitDetails.swift` + `Bullet Tracker/Bullet_Tracker.xcdatamodeld` + any leftover `import CoreData` → build → fix → test → merge.
...
1. `bt-0002` **Wave 4 (widget)** — `BulletTrackerWidgets/BulletTrackerWidgetsBundle.swift` + `WidgetEnums.swift` off Core Data: build a `ModelContainer` in the extension (same App-Group store URL as `DataStore.shared`), rewrite `CompleteHabitIntent` + timeline provider against it. Then delete `Data/CoreDataManager*` + `Bullet Tracker/Bullet_Tracker.xcdatamodeld` → first real Xcode build of the branch → fix → test → merge.
...
- `bt-0002` Wave 3b (commit `bbe8485`, 2026-05-13) — **`DayJournalView` off Core Data.** Last consumer-side file in the bt-0002 scope. Same VM-stored-`modelContext` + full-fetch-then-filter pattern as Wave 3a; `Note(context:)` → `Note(date:content:) + context.insert`. After this only `CoreDataManager*` + the widget still `import CoreData` — both go in Wave 4 + post-Wave-4 deletion. Bead stays `in_progress`; the wave itself is a checkpoint, not a ship.
