...
**Last updated:** 2026-05-13 (`bt-0003` **in build through Wave 4; smoke build clean, device-test pending** — Waves 0 / 1 / 2 / 3 / 3.5 / 4 all shipped to `swiftdata-phase2` branch with commits. Wave 4 rewrote 5 hot-path reads to `FetchDescriptor<HabitEntry>` + `#Predicate` (3 in `HabitStore`, 2 in the widget) — bt-0004's `#Index<HabitEntry>([\.date])` + Wave 2's non-optional `HabitEntry.date` carry the cost. Net +66 lines across 2 files. One spec-departure on `allEntriesByDay` (date-only predicate + Swift-side habit partition, sidestepping IN-predicate on optional UUID through optional relationship — full reasoning in `bt-0003` Wave 4 Decision log). **Device-test still required:** Wave 3.5 lesson holds — `xcodebuild` verifies compilation, not runtime; SwiftData on this iOS version can compile a predicate shape and crash on first read. Device-test surfaces: Today / Dashboard / Journal past-day / Widget rendering / Widget tap. Path 3 hotfix retreat is pre-designed in the Decision log if any reject. CloudKit remains disabled (`cloudKitDatabase: .none`); re-enabled at Wave 5 ship. Punted: `Bullet TrackerTests/Bullet_TrackerTests.swift` cleanup before Wave 5.)
...
- `bt-0003` — SwiftData Phase 2. **In build on `swiftdata-phase2` branch through Wave 4; smoke build clean, device-test pending.** Waves 0 (backup-restore `@MainActor`), 1 (drop dormant machinery + Path 3 recovery validated), 2 (combined schema changes), 3 (consumer rewrites — 15 files), 3.5 (`HabitEntry.details` storage hotfix — computed property over stored `String?` after bt-0004-class SwiftData rejection), 4 (hot-path predicate rewrites — 5 reads across `HabitDataRepository` + widget, +66 lines) all shipped to branch with commits. CloudKit temporarily disabled (`cloudKitDatabase: .none`) for build duration; re-enabled at Wave 5 ship. Wave 4 device-verification is the next gate before Wave 5.
...
1. **bt-0003 Wave 4 device-test.** Build-and-run to phone, exercise the 5 rewritten surfaces (Today / Dashboard / Journal past-day / Widget rendering / Widget tap). Force-close on any = predicate-shape rejection at runtime → Wave 4.5 hotfix (drop `entry.habit?.id == habitID` clause from the predicate, add in-Swift filter — same retreat pattern `allEntriesByDay` already uses; pre-designed in `bt-0003` Wave 4 Decision log).
